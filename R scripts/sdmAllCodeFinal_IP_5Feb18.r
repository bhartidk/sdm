...
#writeRaster(tidalRange.fill, filename="sdm_layers/tidal_range_fill_ip", format="GTiff", overwrite=TRUE)
...
xmin(tidalRange2.fill)

#Saving the raster to disk
#writeRaster(tidalRange2.fill, filename="sdm_layers/tidal_range_minmax_fill_m", format="GTiff", overwrite=TRUE)

##Before starting to handle rasters and shape files, define extents of area of interest. The -35 comes from rounding off the ymin from looking at the sampling locations
...
#rm(coastline, tidalAmp, tidalAmp.con, tidalAmp.ha, tidalAmp.lat, tidalAmp.lon, tidalMax, tidalMax.r, tidalMax_nc, tidalMin, tidalMin.r, tidalMin_nc, tidalRange, tidalRange_nc)

#setting the extent to be used for the rest of the code
ext<-ip.ext
...
names(ms_layers)<-c("bathy_m", "dist_to_shore_km", "bathy_slope_deg", "mean_annual_sss_psu", "sss_freshest_month_psu", "sss_saltiest_month_psu", "mean_annual_sst_C", "sst_coldest_month_C", "sst_warmest_month_C" )
...
writeRaster(ms_layers, filename=names(ms_layers), bylayer=TRUE,format="GTiff", overwrite=TRUE)
setwd("D:/PhD data/Analysis/Chapter2_2Nov17")
...
length(which(is.na(extract(ms_layers[[2]], unique(ip.occ[,2:3])))))
...
names(ms_layers_fill)<-c("bathy_m", "dist_to_shore_km", "bathy_slope_deg", "mean_annual_sss_psu", "sss_freshest_month_psu", "sss_saltiest_month_psu", "mean_annual_sst_C", "sst_coldest_month_C", "sst_warmest_month_C" )
...
setwd("sdm_layers/marspec/ip_ext")
...
#setwd("sdm_layers/worldclim/ip_ext_unedited")
...
#The layer names are removed for some reason when I run the above code.
names(wc_layers_fill)<-c("mean_air_temp_C" , "max_air_temp_C", "min_air_temp_C", "air_temp_dryQ_C")
...
setwd("sdm_layers/worldclim/ip_ext")

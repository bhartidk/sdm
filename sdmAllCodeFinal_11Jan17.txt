...
?varclus
##input occurrence data. This file was written out using env_1Nov17.txt code which used the master_coord_3Nov7.csv input generated from sample_list_vanilla_16Oct17 using R script ecosim_spmat_1Nov17.txt, in Chapter2 R scripts folder
...
##Before starting to handle rasters and shape files, define extents of area of interest. Lets start with handling the undulata data-set and using wg.ext
io.ext<-extent(30,160,-20,50)
wg.ext<-extent(60,90,5,25)

#Saving the extent to be used as a separate variable
ext<-wg.ext

##Loading coastline shape file
coastline<-readOGR(dsn='sdm_layers/CS/ne_10m_coastline', layer='ne_10m_coastline')
crop.coastline<-crop(coastline, ext)
#rm(coastline)

##Loading 200 m bathymetry shapefile
shelf<-readOGR(dsn='sdm_layers/CS/ne_200m_bathymetry', layer='ne_10m_bathymetry_K_200')
crop.shelf<-crop(shelf, ext)
#rm(shelf)

#converting an extent to a polygon - code from https://gis.stackexchange.com/questions/206929/r-create-a-boundingbox-convert-to-polygon-class-and-plot
bound<-as(ext, "SpatialPolygons")
proj4string(bound)<-wgs

#taking the inverse of the crop.shelf polygon
crop.shelf2<- bound-crop.shelf

#plotting everything together
plot(crop.shelf2, col=alpha('khaki', 0.8))
plot(crop.coastline, add=TRUE)

##Removed MEOW shape file - I don't think it is adding anything
...
setwd("sdm_layers/marspec/wg_ext_unedited")
...
#Exporting unedited cropped raster to disk
...
topobath<-raster("sdm_layers/GEBCO2014_topbath_30arcsec_IO.tif")

#resampling topobath so that the resolution and extent match that of ms_layers
tb<-resample(topobath, ms_layers_fill[[1]])
...
plot(tb_slope)
plot(crop.coastline, add=TRUE)
plot(crop.shelf, col=alpha('khaki', 0.2), add=TRUE)

#Writing tb to disk
writeRaster(tb, filename="sdm_layers/topo_bath.tif", format="GTiff", overwrite=TRUE)

#Writing tb_slope to disk
writeRaster(tb_slope, filename="sdm_layers/topo_bath_slope.tif", format="GTiff", overwrite=TRUE)
...
crop.shelf.proj<-spTransform(crop.shelf[1,], CRS(espg.32663))
...
#ms_layers[[3]]
dist.shore<-raster(extent(crop.shelf.proj), nrow=nrow(ms_layers_fill[[1]]), ncol=ncol(ms_layers_fill[[1]]))
...
tidalRange.r<-resample(tidalRange.r, ms_layers_fill[[2]])
...
plot(tidalRange.r) 
plot(crop.coastline, add=TRUE)
points(all.occ[,2:3], pch=19, cex=0.2)

plot(crop.shelf, add=TRUE)

#tidalRange.r counting the number of NAs from extracted values for presence points=4
length(which(is.na(extract(tidalRange.r, unique(all.occ[,2:3])))))
...
writeRaster(tidalRange.fill, filename="sdm_layers/tidal_range_fill_m", format="GTiff", overwrite=TRUE)
...
tidalRange2.r<-resample(tidalRange2.r, ms_layers_fill[[2]])

length(which(is.na(extract(tidalRange2.r, unique(all.occ[,2:3])))))
...
#Saving the raster to disk
writeRaster(tidalRange2.fill, filename="sdm_layers/tidal_range_minmax_fill_m", format="GTiff", overwrite=TRUE)

##Inputing chlorpophyll a concentration at 5 km resolution averaged from 2009 to 2013 using data from  NASA's Ocean Color database. This was done by UNEP World Conservation Monitoring Centre in 2015 (version 1). It is at the resolution of ~5km, but was derived from raw data at ~10 km.

#Description from source - "UNEP-WCMC obtained Aqua MODIS mean sea surface chlorophyll-a data (annual composite at 9 km resolution) in HDF format from the Ocean Color website (http://oceancolor.gsfc.nasa.gov/cgi/l3). The HDF data were converted to ArcGIS rasters using Marine Geospatial Ecology Tools (MGET), and units of chlorophyll-a concentration were converted into milligrams per cubic metre (mg/m3). The composite layer for years 2009 through 2013 was then created using the mosaic function in ArcGIS, to calculate mean chlorophyll-a concentration across all layers"

#Inputting the file
chla_mean<-raster("sdm_layers/chlorophyll/compositechla/w001001.adf")

#Checking if everything looks okay
plot(chla_mean)

#For some reason the extent is a little more than 180 and 90, for max longitude and latitude respectively. Not sure if I should be worried about this - but resampling to ms_layers[[1]] shoulf fix crs, extent 
chla_mean<-resample(chla_mean, ms_layers[[1]])

#setting min and max
chla_mean<-setMinMax(chla_mean)
...
writeRaster(chla_mean, filename="sdm_layers/chla_mean.tif", format="GTiff")

colnames(all.occ)
which(is.na(extract(chla_mean, unique(all.occ[,2:3]))))

##The UNEP chlorophyll data has many NAs - comparing it with Bio-ORACLE
...
bo_layers<-resample(bo_layers, ms_layers_fill[[2]])

#Setting min and max
bo_layers<-setMinMax(bo_layers)

#write unedited raster to disk
setwd("sdm_layers/biooracle/wg_ext_unedited")
writeRaster(bo_layers, filename=names(bo_layers), bylayer=TRUE,format="GTiff")
setwd("D:/PhD data/Analysis/Chapter2_2Nov17")
...
setwd("sdm_layers/biooracle/wg_ext")
writeRaster(bo_layers_fill, filename=names(bo_layers_fill), bylayer=TRUE, format="GTiff", overwrite=TRUE)
setwd("D:/PhD data/Analysis/Chapter2_2Nov17")

###############################################################

ms_list<-list.files(path="sdm_layers/marspec/wg_ext", pattern="*.tif")
ms_layers_fill<-stack()

for(i in 1:length(ms_list)){
ms.raster<-raster(paste0("sdm_layers/marspec/wg_ext/", ms_list[i]))
ms_layers_fill<-stack(ms_layers_fill, ms.raster)
}

#Input wc_layers_fill from disk
wc_list<-list.files(path="sdm_layers/worldclim/wg_ext", pattern="*.tif")
wc_layers_fill<-stack()

for(i in 1:length(wc_list)){
wc.raster<-raster(paste0("sdm_layers/worldclim/wg_ext/", wc_list[i]))
wc_layers_fill<-stack(wc_layers_fill, wc.raster)
}

#Input bo_layers_fill from disk
bo_list<-list.files(path="sdm_layers/biooracle/wg_ext", pattern="*.tif")
bo_layers_fill<-stack()

for(i in 1:length(bo_list)){
bo.raster<-raster(paste0("sdm_layers/biooracle/wg_ext/", bo_list[i]))
bo_layers_fill<-stack(bo_layers_fill, bo.raster)
}

#Input topobath 
tb<-raster("sdm_layers/topo_bath.tif")

#Input topobath slope
tb_slope<-raster("sdm_layers/topo_bath_slope.tif")

#Input distance from 200 m isobath
distShore<-raster("sdm_layers/dist_200m_isobath.tif")

#Input tidalRange.fill
tidalRange.fill<-raster("sdm_layers/tidal_range_minmax_fill_m.tif")

###############################################################

##Selecting predictor variables
#Putting all the raster layers together
candidates<-stack(ms_layers_fill, wc_layers_fill, bo_layers_fill, tb, tb_slope, distShore, tidalRange.fill)
...
#Using a mask such that only points close to the coast are selected - which should be an intersection between wc, ms, bo, tb, tb_slope, tidal_range, dist_shore_200m. Using code from https://stackoverflow.com/questions/5598516/intersection-of-bands-in-r-raster-package to do this
cand.mask <-sum(candidates)

#cutting out the islands as well
#Saving each of the island extents as a polygon within a list and combining those as a SpatialPolygons object
islands<-list()
islands[[1]]<-as(extent(71.77, 73.92, 4.81, 11.8), "SpatialPolygons")
islands[[2]]<-as(extent(79.6, 82.03, 5.84, 9.154), "SpatialPolygons")
islands[[3]]<-as(extent(79.6, 80.96, 9.154, 9.92), "SpatialPolygons")
islands <- do.call(bind, islands) 

plot(crop.coastline)
plot(islands, add=TRUE)

cand.mask<-raster::mask(cand.mask, islands, inverse=TRUE)
plot(cand.mask)

#Using cand.mask as a mask to cinvert cells to NA that don't have values for atleast one of 
...
cor.pts<-randomPoints(candidates, 10000, lonlatCorrection=TRUE)
